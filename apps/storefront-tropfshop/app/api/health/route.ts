 ...
 *
 * Pruefkette: Storefront ist nur „healthy", wenn das Backend healthy ist.
 * Backend-/health prueft DB + Redis (siehe apps/backend/src/api/health/route.ts).
 *
 * 200 wenn Backend-/health 200 liefert; 503 sonst. Body enthaelt die
 * Backend-Antwort als sub-object fuer Diagnose.
 *
 * NEXT_PUBLIC_MEDUSA_BACKEND_URL muss gesetzt sein — wenn nicht, 503 mit
 * Hinweis (gleicher Pfad wie wenn das Backend tot waere).
 */
...
export async function GET() {
  ...
  try {
    ...
    try {
      ...
      // Backend-/health gibt ggf. text/plain (Medusa-Default). Body bleibt null.
    }
    ...
    if (res.ok) {
      ...
      return NextResponse.json(
        { status: "ok", durationMs: totalDurationMs, backend: body ?? { status: "ok" } },
        { status: 200 },
      );
    }
